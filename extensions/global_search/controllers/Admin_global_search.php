...
class Admin_global_search extends Admin_Controller {

	public function index($module = array()) {
		$this->lang->load('global_search/global_search');

		$this->user->restrict('Module.GlobalSearch');
        ...
        if (!empty($module)) {
	        ...
	        if ($this->input->post() AND $this->_updateModule() === TRUE) {
                ...
                redirect('extensions/edit/module/global_search');
            }

            return $this->load->view('global_search/admin_global_search', $data, TRUE);
        }
	}
	...
	private function _updateModule() {
    	...
    	if ($this->validateForm() === TRUE) {
			...
			if ($this->Extensions_model->updateExtension('module', 'global_search', $this->input->post())) {
